    ...
    return (
      <div key={index} className="form__section">
        <Input
          placeholder="Country"
          onChange={(e) =>
            handleInputChange(index, InputOptions.country, e.target.value)
          }
        />
        <Input
          placeholder="City"
          onChange={(e) =>
            handleInputChange(index, InputOptions.city, e.target.value)
          }
        />
  ...
  return (
    <div>
      ...
      <RadioOptions
        ...
        onChangeValue={(item) => setShowForm(item)}
      />
      ...
      {check && (
        <>
          ...
          <RadioOptions
            options={optionsRadio}
            onChangeValue={(item) => setFormShow(item)}
          />
        </>
      )}

      <div className="form__container">
        {formShow === optionsRadio[0] &&
          arrForm.map((_, index) => renderForm(index))}
      </div>
